...
function InswInsertAdjustmentApi($mysqli,$id_transaksi, $tanggal_adjustment, $barang, $jumlah, $jns_dok, $tgl_dok, $no_dok, $nama_pengirim, $keterangan)
{

  //string(10) "TA-0000001" string(10) "2024-06-09" string(6) "100006" string(1) "3" string(7) "0407000" string(10) "2024-06-09" string(18) "103/LDP-ADJUSTMENT" string(5) "Sodik"
  //var_dump($id_transaksi, $tanggal_adjustment, $barang, $jumlah, $jns_dok, $tgl_dok, $no_dok, $nama_pengirim);
  //exit;
  ...
  $tanggalKegiatan = substr($tanggal_adjustment,8,2).'-'.substr($tanggal_adjustment,5,2).'-'.substr($tanggal_adjustment,0,4);
...
// jika user sudah login, maka jalankan perintah untuk insert
  ...
  if (isset($_POST['simpan'])) {
    ...
    $insert = mysqli_query($mysqli, "INSERT INTO tbl_adjustment(id_transaksi, tanggal, barang, stok , total_stok, jumlah, keterangan, jns_dok, tgl_dok, no_dok, nama_pengirim) 
                                     VALUES('$id_transaksi', '$tanggal_adjustment', '$barang', '$stok', '$total_stok','$jumlah', '$keterangan', '$jns_dok', '$tgl_dok', '$no_dok', '$nama_pengirim')")
                                     or die('Ada kesalahan pada query insert : ' . mysqli_error($mysqli));
    // cek query
    // jika proses insert berhasil
    if ($insert) {

      InswInsertAdjustmentApi($mysqli,$id_transaksi, $tanggal_adjustment, $barang, $jumlah, $jns_dok, $tgl_dok, $no_dok, $nama_pengirim, $keterangan);
